...
{
	Vk::CommandBuffer::allocate({ reinterpret_cast<VkCommandBuffer*>(&command_buffers), MaxFramesInFlight });
	...
	for (auto& fence : in_flight_fences) {
		fence = Vk::createFence(VK_FENCE_CREATE_SIGNALED_BIT);
	}
...
void Renderer::update() {
	...
	cmd.insertImageMemoryBarrier(swapchain.images[swapchain.current_image],
	...
	VkRenderingAttachmentInfo color_attachment = {
		...
		.imageView = swapchain.image_views[swapchain.current_image].handle(),
	...
	if (images_in_flight[swapchain.current_image] != VK_NULL_HANDLE) {
		vkWaitForFences(Vk::device(), 1, &images_in_flight[swapchain.current_image], VK_TRUE, UINT64_MAX);
	}
